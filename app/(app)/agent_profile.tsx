...
// --- EditAgentModal is no longer needed ---

// (Types are unchanged)
type Agent = {
  id: string; username: string; email: string | null;
  mobile_no: string | null; address: string | null; aadhar_card_no: string | null;
};
...
export default function AgentProfileScreen() {
  ...
  const fetchData = async () => {
    ...
    if (agentPromise.error) { Alert.alert('Error', 'Failed to fetch agent data'); }
    else { setAgent(agentPromise.data); }
    if (customersPromise.error) { Alert.alert('Error', 'Failed to fetch customer list'); }
  ...
  // (handleDeleteAgent and deleteProfile are unchanged)
...
const styles = StyleSheet.create({
  ...
  listContainer: { marginTop: 20, marginHorizontal: 20, backgroundColor: 'white', borderRadius: 10, elevation: 3, overflow: 'hidden', marginBottom: 30 },
  listTitle: { fontSize: 20, fontWeight: 'bold', color: '#333', padding: 15, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  customerRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 15, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  customerName: { fontSize: 16, fontWeight: 'bold', color: '#333' },
  customerShop: { fontSize: 14, color: '#666' },
  viewButton: { backgroundColor: '#f0f0f0', paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20 },
